...
import { startNativeViewer, stopNativeViewer, stopNativeViewerByIdentity } from "@/src/family-help/native";
import { helperCanView } from "@/src/family-help/stateMachine";
...
export default function ViewFamilyHelp() {
  ...
  useEffect(() => { if (!session || started || !helperCanView("helper", session.state)) return; setStarted(true); void issueSignalingTicket(session.sessionId).then((ticket) => startNativeViewer(session, ticket)); }, [session, started]);
  const generation = session?.generation;
  useEffect(() => () => { if (sessionId && generation) void stopNativeViewerByIdentity(sessionId, generation).catch(() => undefined); }, [generation, sessionId]);
  const leave = async () => { if (session) { await stopNativeViewer(session).catch(() => undefined); await endFamilyHelp(session).catch(() => undefined); } router.replace("/family"); };
