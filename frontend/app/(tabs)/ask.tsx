...
interface Msg { id: string; role: "user" | "higgins"; content: string; pending?: boolean; at: string; conversationId?: string }
interface Handoff { id: string; context: HigginsIssueContext; question: string }
interface FailedRequest extends Handoff { userMsgId: string; higginsMsgId: string }
...
export default function Ask() {
  ...
  const history = useQuery({ queryKey: ["ask-history", deviceId], enabled: !!deviceId, queryFn: () => apiGet<{ id: string; role: "user" | "apollo" | "higgins"; content: string; created_at: string; conversation_id?: string }[]>(`/ask/history?device_id=${deviceId}`) });
  useEffect(() => { if (history.data && messages.length === 0) setMessages(history.data.map((m) => ({ id: m.id, role: m.role === "user" ? "user" : "higgins", content: m.content, at: m.created_at, conversationId: m.conversation_id }))); }, [history.data]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const send = useCallback((msg: string, options?: { handoff?: Handoff; retry?: FailedRequest }) => {
    ...
    const selected = options?.handoff ?? (activeContext && activeHandoffId ? { id: activeHandoffId, context: activeContext, question: clean } : null);
    const retry = options?.retry; const now = new Date().toISOString();
    const initialId = options?.handoff?.id; const conversationId = selected?.id ?? activeHandoffId ?? "general";
    const userMsg: Msg = { id: retry?.userMsgId ?? (initialId ? `u-${initialId}` : `u-${Date.now()}`), role: "user", content: clean, at: now, conversationId };
    const higginsMsg: Msg = { id: retry?.higginsMsgId ?? (initialId ? `h-${initialId}` : `h-${Date.now()}`), role: "higgins", content: "", pending: true, at: now, conversationId };
    ...
    abortRef.current = streamPost("/ask/stream", "ask_apollo", {
      device_id: deviceId, message: clean, conversation_id: conversationId,
      ...(selected?.context ? { context: selected.context } : {}), ...(options?.handoff || retry ? { handoff_id: selected?.id ?? retry?.id } : {}),
    ...
    (err) => {
      ...
      if (err) { setProcessing("failed"); setError(err); if (selected) setFailed({ ...selected, question: clean, userMsgId: userMsg.id, higginsMsgId: higginsMsg.id }); setMessages((items) => items.map((item) => item.id === higginsMsg.id ? { ...item, pending: false } : item)); }
      else setProcessing("completed");
    ...
    return true;
  }, [activeContext, activeHandoffId, deviceId, qc]);

  useEffect(() => {
    const id = params.handoffId ? String(params.handoffId) : "";
    if (!id || id === seenRoute.current || !params.context || !params.prompt || !deviceId) return;
    const context = parseHigginsIssueContext(String(params.context));
    ...
    const handoff: Handoff = { id, context, question: redactUserSecrets(String(params.prompt)).trim() };
  ...
  }, [params.handoffId, params.context, params.prompt, deviceId, send]);
  useEffect(() => () => abortRef.current?.(), []);

  const clearContext = () => { if (streaming) return; setActiveContext(null); setActiveHandoffId(null); setProcessing("idle"); setError(null); setFailed(null); router.setParams({ handoffId: "", context: "", prompt: "" }); };
  const activeMessages = activeHandoffId ? messages.filter((item) => item.conversationId === activeHandoffId) : [];
  const activeQuestions = activeMessages.filter((item) => item.role === "user").length;
  const activeAnswers = activeMessages.filter((item) => item.role === "higgins" && !!item.content).length;
  return <View style={s.root}>
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0}>
      ...
      <FlatList ref={listRef} data={messages} keyExtractor={(item) => item.id} contentContainerStyle={s.list} testID="ask-messages" onContentSizeChange={() => listRef.current?.scrollToEnd({ animated: true })}
        renderItem={({ item }) => <View style={[s.bubble, item.role === "user" ? s.user : s.higgins]} testID={`ask-message-${item.id}`}>
          {item.pending ? <ActivityIndicator color={colors.gold} /> : item.content ? <Text style={s.text}>{item.role === "higgins" ? parseChecks(item.content).text : item.content}</Text> : <Body>Higgins did not finish this answer.</Body>}
          {item.role === "higgins" && !item.pending && item.content ? <><HigginsChecks checks={parseChecks(item.content).checks} askedAt={item.at} messageId={item.id} /><View style={{ marginTop: spacing.sm }}><HigginsSpeakButton text={parseChecks(item.content).text} testID={`ask-hear-${item.id}`} /></View></> : null}
        </View>} />
      {error ? <Card style={s.context} testID="ask-error-card"><Text style={[s.text, { color: colors.barkingText }]} testID="ask-error">{error}</Text>{failed ? <Button testID="ask-retry-button" label="Retry" onPress={() => send(failed.question, { handoff: failed, retry: failed })} disabled={streaming} /> : null}</Card> : null}
