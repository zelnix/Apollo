...
// auto-risky number lists from the app to the extension. Android needs nothing here: the
// CallScreeningService lives in modules/apollo-security and is merged via its own manifest.
//
...
const EXT_NAME = "ApolloCallDirectory";
const SOURCE_FILES = ["CallDirectoryHandler.swift"];
const RESOURCE_FILES = ["callDirectory.json"];
...
const withCallDirectoryTarget = (config) =>
  withXcodeProject(config, (config) => {
    const bundleId = config.ios.bundleIdentifier;
    const extBundleId = `${bundleId}.calldirectory`;
...
const withSourceCheck = (config) =>
  withDangerousMod(config, ["ios", async (config) => {
    const src = path.join(__dirname, "ios", EXT_NAME);
    if (!fs.existsSync(src)) throw new Error(`[withApolloCallGuard] Missing ${src}`);
    return config;
  }]);

module.exports = function withApolloCallGuard(config) {
  config = withSourceCheck(config);
  config = withAppGroupEntitlement(config);
  config = withCallDirectoryTarget(config);
  return config;
};
