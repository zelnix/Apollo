...
export const FILE_SOURCES: { id: FileSource; label: string }[] = [
  ...
  { id: "cloud", label: "Cloud drive / download" }, { id: "nearby", label: "AirDrop / Nearby Share" }, { id: "unknown", label: "Not sure" },
];
...
export function analyseFile(f: FileInput): FileAnalysis {
  ...
  const sourceEvidence = f.source === "cloud" ? "Being hosted on Google Drive or another cloud service does not verify the file, its contents or its sender." : null;
  ...
  const R = (scenario: string, title: string, state: ApolloState, verdict: string, why: string[], recommendation: string, handoff: FileAnalysis["handoff"] = "none"): FileAnalysis => ({ scenario, title, state, verdict: inspected ? verdict : `I couldn't inspect this file. ${title}: based only on the supplied name and context.`, why: inspected ? [...why, ...(sourceEvidence ? [sourceEvidence] : []), 'Limited signature/sample check only. Other content, compressed data and links outside the sample were not inspected.'] : [`The name claims ${claimedType}. No content was read; type and safety are unknown.`, ...(sourceEvidence ? [sourceEvidence] : [])], recommendation, realType: inspected ? real : 'unknown', claimedType, urls, handoff, technical });
