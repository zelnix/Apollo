...
export function analyseNetwork(input: NetworkInput): NetworkAnalysis {
  ...
  const technical = [`Connection: ${n ? (n.connected ? n.type : "offline") : "unknown"}`, ssid ? `Network name: ${ssid}` : "Network name: not revealed by the platform", wifi ? `Wi‑Fi security: ${n?.wifiSecurity}` : "", `Captive portal: ${n?.captivePortal === null || n?.captivePortal === undefined ? "unknown" : n.captivePortal ? "yes" : "no"}`, `VPN: ${vpnOn ? "active" : n?.vpnActive === null ? "unknown" : "off"}`, `Inspectable by this build: ${n?.inspectable ? "yes" : "no"}`,
    sdk ? `Available device signals: ${sdk.blockedMalicious} reported stops, ${sdk.unknownHosts} unknown destinations${sdk.c2Apps.length ? `, dangerous traffic from ${sdk.c2Apps.join(", ")}` : ""}${sdk.dnsChanged ? ", DNS changed" : ""}` : "Detailed network events are not available on this build", `Context: ${input.context}${trusted ? " (trusted network)" : ""}`, `Threat Scent: ${scent.size ? [...scent].join(", ") : "none"}`].filter(Boolean);
  ...
  if (sdk?.c2Apps.length) return R("N07", "Dangerous traffic reported from an app", "barking", `Available device signals associate ${sdk.c2Apps.join(", ")} with known dangerous infrastructure. This summary does not provide enough evidence to confirm that Apollo blocked each connection.`, ["This is not a one-off bad link: the app was associated with repeated dangerous destinations.", "Review the app and the underlying event evidence before claiming enforcement."], "Open Check This App and remove it unless you can explain why it needs that traffic.", "app");
  // N06 / N12 — dangerous connection already blocked; calm.
  if (sdk?.blockedMalicious) return R("N12", "Dangerous connection reported", "barking", `Available device signals report ${sdk.blockedMalicious === 1 ? "one dangerous connection" : `${sdk.blockedMalicious} dangerous connections`}, but this aggregate summary does not contain confirmed enforcement evidence for each event.`, ["The destination count came from an available device signal.", "Apollo only says a connection was blocked when the event includes confirmed enforcement evidence."], "Review recent connection events for confirmed actions and identify the app involved.");
  ...
  if (wifi) return R("N02", "Wi‑Fi you haven't classified", "ears_up", `You're on ${ssid ? `“${ssid}”` : "a Wi‑Fi network"}. Choose whether it is home, work or public so Apollo can apply the right checks.`, ["Unfamiliar networks get a closer watch by default.", "Nothing suspicious has been seen on it."], "Pick a context above, or mark it trusted in Gates if it's yours.");
  if (!n.inspectable || n.type === "unknown") return R("N02", "Network details unavailable", "ears_up", "Apollo can see that a connection exists, but this build cannot inspect enough network details to call it clear.", ["Connection type or Wi‑Fi details are unavailable.", "No dangerous destination was confirmed, and no protective block was confirmed."], "Avoid sensitive activity until you can identify the network or switch to a connection you trust.");
