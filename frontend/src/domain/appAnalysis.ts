...
export function analyseApp(input: AppInput): AppAnalysis {
  ...
  const technical = [`Name: ${name}`, input.developer ? `Developer: ${input.developer}` : "Developer: not provided", `Source: ${APP_SOURCES.find((s) => s.id === input.source)?.label}`, `Claimed purpose: ${APP_PURPOSES.find((p) => p.id === input.purpose)?.label}`,
    ...
    `Remote-access capable: ${remoteCapable ? "yes" : "no"}`, brand ? `Name contains brand: ${brand}` : "", net ? `Available network signals: ${net.blockedMalicious} reported stops, ${net.unknownHosts} unknown destinations` : "Network behaviour: not visible on this build", `Threat Scent context: ${scent.size ? [...scent].join(", ") : "none"}`, `Risk score: ${score}`].filter(Boolean);
  ...
  if (net?.blockedMalicious) return R("A14", "Dangerous connection signal", byScore === "barking" ? "barking" : "growling", `Available device signals associate ${name} with a dangerous destination. This summary does not prove the app is malicious or confirm an Apollo block.`, [`${net.blockedMalicious} connection${net.blockedMalicious > 1 ? "s were" : " was"} reported against known malicious infrastructure.`, "An app that talks to dangerous servers isn't necessarily malware itself — but it deserves a hard look.", ...permWhy], "Review the underlying network event. If you don't recognise or need this app, remove it; otherwise check for an update from the official store.", { handoff: "network" });
  ...
  if (net && net.unknownHosts >= 5) return R("A15", "Talks to many unknown servers", "ears_up", `${name} connects to ${net.unknownHosts} destinations Apollo doesn't recognise. None are confirmed dangerous.`, ["Ads and analytics explain most of this for free apps.", "Apollo isn't crying wolf — it's just noting the pattern.", "Nothing was blocked."], "No action needed now. Re-check Network Gate if the app behaves unexpectedly or its connections change.", { handoff: "network" });
  ...
  if (official) return R(input.purpose === "security" ? "A13" : "A18", input.purpose === "security" ? "Security app reported from the official store" : "App reported from the official store", "ears_up", `The available information did not establish a specific concern about ${name}, but a store source and selected permissions do not prove safety.`, ["The source is a useful signal — not proof of safety or developer identity.", perms.length ? "The supplied or observed permissions fit the selected purpose." : "No sensitive permissions were supplied or observed; unavailable permissions remain unknown.", "No malicious behaviour was established by the evidence available to this check."], "No urgent action from this evidence alone. Re-check if its permissions change or it behaves unexpectedly.");
  // not_sure source, nothing else unusual.
  return R("A18", "App from an unknown source", "ears_up", `No concerning capability was supplied or observed for ${name}, but its source and any unavailable permissions remain unknown.`, ["Where an app came from matters as much as what it asks for.", "Check its store listing: open the App Store / Google Play and search for it.", "If it isn't there, treat it as sideloaded."], "Look it up in the official store. If you can't find it, consider removing it.");
}
