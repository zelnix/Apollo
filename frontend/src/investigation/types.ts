...
export interface ActionProposal { id: string; kind: "instruction" | "open_settings" | "request_permission" | "recheck" | "open_verified_source"; label: string; instruction: string; capabilityId: string | null; executionDescriptorId: string | null; requiresUserGesture: boolean; sourceIds: string[] }
...
export interface DeviceProfile { platform: "android" | "ios" | "windows" | "macos" | "web"; manufacturer: string | null; model: string | null; osVersion: string | null; locale: string; evidenceOrigin: "native" | "user_reported" | "browser"; capabilityIds: string[] }
...
export interface DeviceResult { requestId: string; caseRevision: number; capabilityId: string; status: "observed" | "unavailable" | "permission_required" | "denied" | "failed"; observedAt: string | null; values: Record<string, string | number | boolean | string[] | null>; simulation: { kind: "preview_device"; label: string; fixtureId: string } | null }
