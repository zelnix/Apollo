...
const LOCAL_OPERATION_LIFETIME_MS = 15 * 60 * 1000;
...
export interface ManagedOperation {
  ...
  caseId: string | null;
  kind: "create" | "append";
  input: Omit<CreateCase, "deviceProfile"> | null;
  files: OperationFile[];
  fileIndex: number;
...
function scheduleExpiry(record: ManagedOperation) {
  ...
  if (["expired", "cancelled"].includes(record.phase)) return;
...
function bindCase(record: ManagedOperation, caseData: InvestigationCase) {
  ...
  const expiresAt = new Date(Math.min(Date.parse(record.expiresAt), Date.parse(caseData.expiresAt))).toISOString();
  return publish(record, { caseData, revision: caseData.revision, expiresAt });
}
...
function assertLive(record: ManagedOperation, generation: number): void {
  if (operations.get(record.operationId) !== record || record.generation !== generation || ["expired", "cancelled"].includes(record.phase)) {
    throw new Error("This investigation operation is no longer active.");
  }
  if (Date.parse(record.expiresAt) <= Date.now()) {
    expireOperation(record.operationId, record.caseId);
    throw new Error("This temporary investigation operation expired.");
  }
}

function terminate(record: ManagedOperation, phase: "expired" | "cancelled", error: string | null) {
  record.generation += 1;
  releaseFiles(record);
  const timer = timers.get(record.operationId);
  if (timer) clearTimeout(timer);
  timers.delete(record.operationId);
  const caseId = record.caseId;
  record.input = null;
  record.turn = null;
  record.evidenceIds = [];
  record.caseData = null;
  record.job = null;
  record.running = null;
  publish(record, { phase, error });
  operations.delete(record.operationId);
  if (caseId) operationPairs.delete(pairKey(record.operationId, caseId));
}

async function runCreate(record: ManagedOperation): Promise<ManagedOperation> {
  const generation = record.generation;
  assertLive(record, generation);
  if (!record.input || !record.turn) return record;
  if (!record.caseData) {
    ...
    if (operations.get(record.operationId) !== record || record.generation !== generation) {
      void api.deleteCase(opened.case.id).catch(() => undefined);
      return record;
    }
    assertLive(record, generation);
...
function execute(record: ManagedOperation): Promise<ManagedOperation> {
  ...
  if (["submitted", "settled", "expired", "cancelled"].includes(record.phase)) return Promise.resolve(record);
  if (Date.parse(record.expiresAt) <= Date.now()) { expireOperation(record.operationId, record.caseId); return Promise.resolve(record); }
...
export function retryManagedOperation(operationId: string, caseId?: string | null) {
  ...
  return record?.phase === "failed" ? execute(record) : Promise.resolve(null);
}
...
export function expireOperation(operationId: string, caseId?: string | null) {
  const record = requireOperation(operationId, caseId);
  if (!record) return;
  terminate(record, "expired", "This temporary investigation operation expired.");
}
