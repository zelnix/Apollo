...
/** Capability the descriptor can observe after return, or null when only the person can confirm. */
...
/** Completes a pending attempt with a fresh observation. Safe to call more than once; the attempt is consumed. */
export async function completeAttempt(attempt: ActionAttempt): Promise<RecheckOutcome> {
  await clearAttempt();
  const returned: ActionAttempt = { ...attempt, returnedAt: new Date().toISOString(), status: "returned" };
  const capabilityId = observationCapabilityFor(attempt.descriptorId);
  if (!capabilityId) return { attempt: returned, observation: null, evidenceId: null, plan: attempt.planId ? { outcome: "cannot_observe", explanation: "Apollo cannot read this setting on this device; only you can confirm what you changed." } : null };
  try {
    const fresh = await api.getCase(attempt.caseId);
    if (["cancelled", "expired", "failed"].includes(fresh.case.status)) return { attempt: { ...returned, status: "failed" }, observation: null, evidenceId: null, plan: null };
    ...
    return { attempt: { ...returned, status: "failed" }, observation: null, evidenceId: null, plan: { outcome: "failed", explanation: e instanceof Error ? e.message : "The fresh check could not be completed." } };
  }
}
