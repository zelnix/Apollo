...
test("F20 fake WhatsApp update APK → barking", () => { const r = run("WhatsApp_update.apk", "message"); assert.equal(r.state, "barking"); assert.match(r.verdict, /couldn't inspect this file|never update through a file/i); });
...
test("F11 configuration profile → barking", () => { const r = run("wifi-setup.mobileconfig", "message"); assert.equal(r.state, "barking"); assert.match(r.verdict, /couldn't inspect this file|managed|network/i); assert.equal(r.handoff, "network"); });
...
test("F16 / acceptance 5: unknown extension → ears_up, no bark", () => { const r = run("data.qzx", "email"); assert.equal(r.state, "ears_up"); assert.match(r.verdict, /couldn't inspect this file|don't know this file type/i); });
