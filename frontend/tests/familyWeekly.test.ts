...
test("alerts all handled → ears_up, nothing open", () => { const h = weeklyHeadline(base({ total: 5, alerts: 2, handled_alerts: 2 }), NOW); assert.equal(h.tone, "ears_up"); assert.match(h.text, /2 alerts .* handled them all\. Quite so\./); });
test("open alert → growling, suggests a call", () => { const h = weeklyHeadline(base({ total: 5, alerts: 1, open_alerts: 1 }), NOW); assert.equal(h.tone, "growling"); assert.match(h.text, /1 alert still open .* I would suggest a call/); });
...
test("last seen labels", () => {
  ...
  assert.equal(lastSeenLabel(base({ last_seen_at: null }), NOW), "Apollo has not yet heard from their phone.");
});
