...
class FamilyAssistProjectionService : Service() {
  ...
  override fun onCreate() { super.onCreate(); FamilyAssistApp.context = applicationContext; createChannel() }
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    when (intent?.action) {
      ...
      ACTION_STOP -> stopOwned("owner_stopped")
    }
  ...
  private fun start(intent: Intent) {
    ...
    val projection = manager.getMediaProjection(intent.getIntExtra(EXTRA_RESULT_CODE, Activity.RESULT_CANCELED), data) ?: return stopOwned("capture_failed")
    projection.registerCallback(object : MediaProjection.Callback() { override fun onStop() { stopOwned("capture_revoked") } }, android.os.Handler(mainLooper))
    ...
    peer = FamilyAssistRtcPeer(values, true, track).also { it.connect() }
    FamilyAssistRuntime.state["startedAt"] = FamilyAssistRuntime.isoNow(); FamilyAssistRuntime.update("starting")
  }
  ...
  @Synchronized private fun stopOwned(reason: String) {
    ...
    peer?.close(); peer = null; source?.dispose(); source = null; track?.dispose(); track = null
    ...
    FamilyAssistRuntime.update("stopped", "capture_stopped", null); stopForeground(STOP_FOREGROUND_REMOVE); stopSelf()
  }
  ...
  override fun onDestroy() { if (capturer != null) stopOwned("app_terminated"); FamilyAssistApp.context = null; super.onDestroy() }
}