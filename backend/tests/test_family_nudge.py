...
class TestNudgeScheduler:
    def test_only_tuesday_window_and_only_missed(self):
        ...
        async def run():
            # Fresh Motor client bound to THIS event loop — see the identical comment in
            # test_family_weekly_push.py::test_window_and_week_key for why this is needed (both
            # files monkey-patch the same shared `family_weekly.db` singleton across separate
            # asyncio.run() calls, which otherwise raises "Event loop is closed").
