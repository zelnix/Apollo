...
class StorageError(Exception):
    def __init__(self, status: int, detail: str, outcome: str = "unknown"):
...
async def put_object(path: str, data: bytes, content_type: str) -> dict:
    ...
    if not storage_configured():
        raise StorageError(503, "Family voice storage is not configured, so the upload was not attempted.", "not_stored")
    try:
        ...
        raise StorageError(503, f"Family voice storage did not confirm the upload ({type(exc).__name__}). Apollo will verify and remove any remote copy.", "unknown") from exc
