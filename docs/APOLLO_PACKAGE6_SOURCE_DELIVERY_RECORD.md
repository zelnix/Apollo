...
- WFP classify-drop events are subscribed and emitted as bounded one-event JSON files with destination, protocol, actual WFP filter ID and WFP-provided application/process attribution. These are the only Windows records eligible for verified enforcement evidence.
...
- Added an `OSSystemExtensionRequest` activation/deactivation helper, system-extension and App Group entitlements, an XcodeGen project and repeatable preparation/embedding scripts.
...
- “Threat stopped” remains evidence-only. Source presence, installation, activation intent and a stored rule are not enforcement evidence.
...
- ESLint: pass.
- Python lint: pass.
- Frontend Node source/unit tests: **379 passed**.
